...
def write_to_google_sheet(data, spreadsheet_id, sheet_name):
    try:
        ...
        headers = ["Date", "Customer", "Link",    "Drive Time", "Time Spent on Job",    "Final Invoice", "Our Cost On Parts", "Tech Burden to run call", "Day rate per job Cost",  "Commision Paid To Tech","Daily Revenue  (+/-):"]
        if current_headers:
        ...
        if row_to_update:
            ...
            newdata = []  # Initialize an empty list
            newdata.append(data[0])
            newdata.append(data[1])
            newdata.append(data[2])
            newdata.append(driven_time)
            newdata.append(time_spent)
            newdata.append(existing_row[5])
            newdata.append(existing_row[6])
            newdata.append(tech_Burden_to_run_call)
            newdata.append(existing_row[8])
            newdata.append(existing_row[9])
            newdata.append(existing_row[10])
            print(newdata)
            # Update the existing row
            cell_range = f'A{row_to_update}:K{row_to_update}'
            sheet.update(cell_range, [newdata])
            next_row = row_to_update
        else:
            # Initialize newdata for the else case
            newdata = []
            newdata[0] =  data[0]
            newdata[1] = data[1]
            newdata[2] = data[2]
            newdata[3] = driven_time
            newdata[4] = time_spent
            newdata[7] = tech_Burden_to_run_call
            next_row = len(existing_rows) + 1
            sheet.append_row(newdata)

        if driven_time < 20:
            cell = sheet.cell(next_row, 4)  # J column is the 10th column, 1-indexed as 10
            cell_format = {
                "backgroundColor": {
                    "red": 0,
                    "green": 1,
        ...
        if time_spent < 45:
            cell = sheet.cell(next_row, 5)  # J column is the 10th column, 1-indexed as 10
            cell_format = {
                "backgroundColor": {
                    "red": 0,
                    "green": 1,
